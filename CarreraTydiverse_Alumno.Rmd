...
date:  "27/03/2024"  #Pondría la fecha del día actual
params:
  lang: ES
...
```{r}
cran2[order(cran2$ip_id, cran2$package),]
```
13. Ordena `cran2` según los valores de `country` (ascendente), `r_version` (desdencente) e `ip_id`(ascendente).
```{r}
cran2[order(cran2$country, cran2$r_version, cran2$ip_id),decreasing=F,T,F]
```
...
```{r}
cran3 %>% mutate (size_mb=size/2^10)
```
...
```{r}
valor_medio=by_package%>% summarise(valor_medio=mean(size))
valor_medio
